...
sealed abstract class InjectionCands[C <: blackbox.Context](val c: C)(val cands: Vector[C#Symbol]) {
  ...
  def rankingEvaluation: (c.Expr[InjectionPriority], Seq[C#Symbol]) = {
    cands.map { sm =>
      val ip = sm.annotations.find(_.tree.tpe =:= InjectionPriorityConfigType)
        ...
        .fold[c.Expr[InjectionPriority]](c.universe.reify(Default)) {
          ...
          case _ => c.abort(c.enclosingPosition, s"The injection priority setting must be a static module. From [ ${sm.fullName} ]")
        }
      ip -> sm
    }.groupBy(x => c.eval(x._1)).toSeq.minBy(_._1)(InjectionPriority.Order)._2
    }.toList match {
...
case class ConfirmedCands[C <: blackbox.Context](override val c: C)(cands: Vector[C#Symbol]) extends InjectionCands[C](c)(cands)

case class ExcludingRuntime[C <: blackbox.Context](override val c: C)(cands: Vector[C#Symbol]) extends InjectionCands[C](c)(cands)
