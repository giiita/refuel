  ...
  def generateExpr[T: c.WeakTypeTag](targets: Seq[C#Symbol]): C#Expr[T] = {
    targets
      .map { cnd =>
        cnd -> {
          cnd.annotations
            .find(_.tree.tpe.=:=(EffectiveConfigType))
      ...
      case (annotated, non) =>
        ...
        c.echo(
          c.enclosingPosition,
          s"Effect is inspected in Runtime. These are inspected [${annotated.map(_._1.fullName).mkString(",")}]. Use ${non.headOption
            .map(_._1.fullName)} if everything is invalid."
        )
            ...
            case x if x.isEmpty   =>
              ...
              if (_nonAnnotatedCands.isEmpty) {
                throw new InjectDefinitionException(
                  s"Can't find a dependency registration. Injection from runtime classpath must be given @RecognizedDynamicInjection."
                )
              } else {
