...
import com.phylage.scaladia.internal.PropertyDebugModeEnabler
import com.phylage.scaladia.json.error.{DeserializeFailed, UnexpectedDeserializeType}
...
class CaseCodecFactory(val c: blackbox.Context) extends PropertyDebugModeEnabler {
  ...
  protected def recall[T](q: WeakTypeTag[T]): c.Expr[Codec[T]] = c.Expr(
    q"com.phylage.scaladia.json.codecs.factory.CaseClassCodec.from[$q]"
  )
        ...
        override def serialize(t: T): Json = {
          ...
          c.Expr[Json](
            q"""
             ...
             com.phylage.scaladia.json.entry.JsObject.apply(
