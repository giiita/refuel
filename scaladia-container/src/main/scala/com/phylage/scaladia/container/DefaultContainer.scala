...
import com.phylage.scaladia.injector.scope.{CanBeRestrictedSymbol, IndexedSymbol, TypedAcceptContext}
import com.phylage.scaladia.internal.AtomicUpdater
...
private[scaladia] object DefaultContainer {
...
private[scaladia] class DefaultContainer private(val lights: Vector[Container] = Vector.empty)
  ...
  @volatile private[scaladia] var _buffer: ContainerPool = TrieMap.empty
   ...
   * @return
   */
  private[scaladia] final def cache[T](value: IndexedSymbol[T]): IndexedSymbol[T] = {
   ...
   * @return
   */
  private[scaladia] override def createScope[T: universe.WeakTypeTag](x: T, priority: Int): IndexedSymbol[T] = {
