...
class PredefCodecsTest extends AsyncWordSpec with Matchers with Diagrams with Injector {
  ...
  "Reserved type codec auto translation" should {
    "json rw" in {
    ...
    "zoned date time rw with epoch" in {
      implicit val tz: RuntimeTZ = inject[RuntimeTZ]
      implicit val str: DateTimeConversionStrategy = DateTimeConversionStrategy.Epoch
      val row = ZonedDateTime.of(2021, 10, 1, 10, 0, 0, 0, tz.ZoneId)
    ...
    "zoned date time rw with epoch millis" in {
      implicit val tz: RuntimeTZ = inject[RuntimeTZ]
      implicit val str: DateTimeConversionStrategy = DateTimeConversionStrategy.EpochMillis
      val row = {
        val date = ZonedDateTime.now(tz.ZoneId)
        date.withNano(0)
      }
    ...
    "zoned date time rw with format" in {
      implicit val tz: RuntimeTZ = inject[RuntimeTZ]
      implicit val str: DateTimeConversionStrategy = DateTimeConversionStrategy.Formatter
      val row = ZonedDateTime.of(2021, 10, 1, 10, 0, 0, 0, tz.ZoneId)
    ...
    "Try (Failure) rw" in {
      ...
      tree.readAs[tpe].fold(fail(_), _ shouldBe Failure(IllegalJsonFormatException("Cannot deserialize 3.2 into a Boolean. Only 1/0 or true/false can be boolean decoded.")))
