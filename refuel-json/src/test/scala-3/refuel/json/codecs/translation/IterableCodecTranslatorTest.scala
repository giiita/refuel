...
class IterableCodecTranslatorTest extends AsyncWordSpec with Matchers with Diagrams with Injector {
  ...
  "Declarative codec definition translation" should {
    ...
    "case List" in {
      ...
      val read = ReadOf.list[List[String]]
      read.deserialize(json) shouldBe row
    ...
    "case Vector" in {
      ...
      val read = ReadOf.vector[Vector[String]]
      read.deserialize(json) shouldBe row
    ...
    "case Set" in {
      ...
      val read = ReadOf.set[Set[String]]
      read.deserialize(json) shouldBe row

      val write = WriteOf.set[Set[String]]
      write.serialize(row) shouldBe json
    }
    ...
    "case Array" in {
      ...
      val read = ReadOf.array[Array[String]]
      read.deserialize(json) shouldBe row

      val write = WriteOf.array[Array[String]]
      write.serialize(row) shouldBe json
    }
    ...
    "case Option" in {
      ...
      val read = ReadOf.option[Option[String]]
      read.deserialize(json) shouldBe row

      val write = WriteOf.option[Option[String]]
      write.serialize(row) shouldBe json
    }
